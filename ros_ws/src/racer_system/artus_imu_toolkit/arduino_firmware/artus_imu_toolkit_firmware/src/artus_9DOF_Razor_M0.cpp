...
artus_9DOF_Razor_M0::artus_9DOF_Razor_M0(){//} : filter_(filter) {
...
int artus_9DOF_Razor_M0::imu_init(){
    /// Set up LED pin and turn off (active-high, default to off)
    pinMode(HW_LED_PIN, OUTPUT);
    digitalWrite(HW_LED_PIN, LOW);
    // !ignoring SD Card Hardware for (hopfully) save some energy! //
    /// Call imu.begin() to verify communicaton as well as initializing the IMU (artus_9DOF_Razor_M0) to it's default values
    /// On Succes this function will return `INV_SUCESS`(0), that indicates, that the IMU of given type was set up succesfully.
    return imu.begin();
}

int artus_9DOF_Razor_M0::imu_start(){
...
int artus_9DOF_Razor_M0::updatedIMUvalues(){
...
int artus_9DOF_Razor_M0::updatedIMUvalues(imuDataContainer & imuValues){
    if ( !imu.dataReady() ) {
        return 1;
    }
    /// Call update() to update the imu objects sensor data. You can specify which sensors to update by combining:
...
int artus_9DOF_Razor_M0::getRawAcceleration(vec3imu& accelRawValues){
...
int artus_9DOF_Razor_M0::getRawGyro(vec3imu& gyroRawValues){
...
int artus_9DOF_Razor_M0::getRawMagnetometer(vec3imu& magRawValues){
...
int artus_9DOF_Razor_M0::getTemperature(float& temperatureValue){
...
int artus_9DOF_Razor_M0::getCorAcceleration(vec3imu& accelCorValues, const vec3imu& accelRawValues){
// ===== Value Correction ============================================================================================
...
int artus_9DOF_Razor_M0::getCorGyro(vec3imu& gyroCorValues , const vec3imu& gyroRawValues){
// ===== Value Correction ============================================================================================
...
int artus_9DOF_Razor_M0::getCorMagnetometer(vec3imu& magCorValues, const vec3imu& magRawValues){
// ===== Value Correction ============================================================================================
...
int artus_9DOF_Razor_M0::getEulerAngle(float& roll,float& pitch, float& yaw){
...
int artus_9DOF_Razor_M0::getEulerAngle(euler3imu& eulerAngle){
    return getEulerAngle(eulerAngle.roll,eulerAngle.pitch,eulerAngle.yaw);
}

int artus_9DOF_Razor_M0::getQuaternion(float& qw, float& qx, float& qy, float& qz ){
    filter_.getQuaternion(&(qw), &(qx), &(qy), &(qz));
    return 0;
}

int artus_9DOF_Razor_M0::getQuaternion(quaternion4imu&  quaternion){
    return getQuaternion(quaternion.qw, quaternion.qx, quaternion.qy, quaternion.qz);
}

int artus_9DOF_Razor_M0::getLinearAcceleration(vec3imu& linearAccel, vec3imu& gravity , const vec3imu& accelCorValues){
    getGravity(gravity);
    linearAccel = (accelCorValues - gravity) * SENSORS_g_TO_MS2;  //~> to [m/s²]
    return 0;
}

int artus_9DOF_Razor_M0::getAngularVelocity(vec3imu& angularVel , const vec3imu& gyroCorValues){
    angularVel = gyroCorValues * SENSORS_DPS_TO_RADS;  //~> [rad/s]
    return 0;
}

int artus_9DOF_Razor_M0::getRelativSpeed(vec3imu& relSpeed, vec3imu& acceleration_cur, vec3imu& acceleration_prev, const unsigned long& curTimeStamp , const unsigned long& prevTimeStamp ){
...
int artus_9DOF_Razor_M0::getRelativeDistance(vec3imu& relDistance,vec3imu& relVelocity_cur, vec3imu& relVelocity_prev, const unsigned long& curTimeStamp , const unsigned long& prevTimeStamp ){
    if (curTimeStamp == prevTimeStamp){
        return 1;
    }
...
int artus_9DOF_Razor_M0::getTimeStamp(unsigned long& arduTimeStamp){
    arduTimeStamp = millis();
    return 0;
}

int artus_9DOF_Razor_M0::getGravity(vec3imu& gravity) {

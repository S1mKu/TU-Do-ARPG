...
from hands_srv.srv import Gap_Follower, Gap_FollowerResponse
...
class GapFollower:
    BUBBLE_RADIUS = 180
    PREPROCESS_CONV_SIZE = 4
    BEST_POINT_CONV_SIZE = 200
    MAX_LIDAR_DIST = 1500000
    STRAIGHTS_SPEED = 3.0 # 20.0
    CORNERS_SPEED = 3.0
...
def calculate_commands(req):

    scan = req.scan

    gf = GapFollower()

    speed, steering_angle = gf.process_lidar(scan.ranges)

    res = Gap_FollowerResponse()
    ...
    res.steering_angle = steer_msg
    res.velocity = vel_msg

    return res

def server():
    rospy.init_node('gap_follower_server')
    s = rospy.Service('calculate_commands', Gap_Follower, calculate_commands)
    print("Ready to provide control command calculation.")
    rospy.spin()

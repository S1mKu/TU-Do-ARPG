...
static const std::string scan_topic_ = "lidar";
...
class AmclNode
{
  ...
  private:
    ...
    ros::Publisher pose_with_scan_pub_;
        ...
        first_reconfigure_call_(true)
{
  ...
  odom_frame_id_ = frame_prefix + stripSlash(odom_frame_id_);
  base_frame_id_ = frame_prefix + stripSlash(base_frame_id_);
  global_frame_id_ = "/" + stripSlash(global_frame_id_);

  ROS_WARN_STREAM("AMCL odom_frame_id_: " << odom_frame_id_);
  ROS_WARN_STREAM("AMCL base_frame_id_: " << base_frame_id_);
  ...
  pose_with_scan_pub_ = nh_.advertise<eyes_msgs::PoseWithScan>("pose_with_scan", 2, true);
...
{
  // ROS_WARN_STREAM("AmclNode::laserReceived - start");
  ...
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end()) // && stripSlash(laser_scan_frame_id).compare(stripSlash(base_frame_id_)) != 0)
  {
    // ROS_WARN_STREAM("AmclNode::laserReceived - frame_to_laser_ 1");
    ...
    // ROS_WARN_STREAM("AmclNode::laserReceived - frame_to_laser_ 2");
  ...
  if(pf_init_)
  {
    // ROS_WARN_STREAM("AmclNode::laserReceived - if(pf_init_)");
  ...
  {
    // ROS_WARN_STREAM("AmclNode::laserReceived - laser_update_");
    ...
    ROS_WARN_STREAM("AMCL - Num samples: " << set->sample_count);
  ...
  {
    // ROS_WARN_STREAM("AmclNode::laserReceived - resampled");
    ...
    if(max_weight > 0.0)
    {
      ...
      eyes_msgs::PoseWithScan pose_with_scan;
      pose_with_scan.header = p.header;
      pose_with_scan.pose = p.pose;
      pose_with_scan.scan = *laser_scan;

      pose_with_scan_pub_.publish(pose_with_scan);
      ...
      {
        // ROS_WARN_STREAM("AmclNode::laserReceived - transform 1");
        ...
        // geometry_msgs::TransformStamped map_odom_msg;
        // map_odom_msg.header.frame_id = global_frame_id_;
        // map_odom_msg.header.stamp = transform_expiration;
        // map_odom_msg.child_frame_id = odom_frame_id_;
        // map_odom_msg.transform.translation.x = 0.0;
        // map_odom_msg.transform.translation.y = 0.0;
        // map_odom_msg.transform.translation.z = 0.0;
        // map_odom_msg.transform.rotation.x = 0.0;
        // map_odom_msg.transform.rotation.y = 0.0;
        // map_odom_msg.transform.rotation.z = 0.0;
        // map_odom_msg.transform.rotation.w = 1.0;
        // this->tfb_->sendTransform(map_odom_msg);
  ...
  else if(latest_tf_valid_)
  {
    if (tf_broadcast_ == true)
    {
      // ROS_WARN_STREAM("AmclNode::laserReceived - transform 2");
  ...
  // ROS_WARN_STREAM("AmclNode::laserReceived - end");
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  else if("/" + stripSlash(msg.header.frame_id) != "/" + stripSlash(global_frame_id_))
  {
  ...
  ROS_WARN("Setting pose (%.6f): %.3f %.3f %.3f",
           ros::Time::now().toSec(),

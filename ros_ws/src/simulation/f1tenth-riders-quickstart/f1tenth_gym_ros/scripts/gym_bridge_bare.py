...
from f1tenth_gym_ros.msg import Observation, RaceInfo

current_dir = os.path.abspath(os.path.dirname(__file__))
package_dir = os.path.abspath(os.path.join(current_dir, ".."))

RENDERING_ENABLED = os.environ.get(
    "F1TENTH_RENDERING", "False").lower() == "true"
...
class Agent(object):
    def __init__(self, id, drive_callback, scan_fov, scan_beams):
        ...
        self.observations_pub = rospy.Publisher(
            self.observations_topic, Observation, queue_size=1)
        self.drive_sub = rospy.Subscriber(
            self.drive_topic, AckermannDriveStamped, drive_callback, queue_size=1)
        self.scan_pub = rospy.Publisher(
            self.scan_topic, LaserScan, queue_size=1)
    ...
    def publish_observations(self, ts):
        ...
        observation.ego_pose, observation.ego_twist = _to_odometry_msg(
            **self.ego_obs)
...
class InfoPrinterPlugin(BridgePlugin):
    ...
    def __init__(self, drive_timer_duration):
        ...
        self.PRINT_EVERY_TIMER_TICK = int(
            self.PRINT_EVERY_SECONDS / drive_timer_duration)
        ...
        self.info_pub = rospy.Publisher(self.race_info_topic, RaceInfo, queue_size=2)
    ...
    def on_update(self, agents, obs, done):
        ...
        if self.timer_tick_count == self.PRINT_EVERY_TIMER_TICK:
            ...
            if not self.ego_collision:
                self.ego_collision = bool(obs['collisions'][0])
            # if not self.opp_collision:
            #     self.opp_collision = bool(obs['collisions'][1])
            info.ego_collision = self.ego_collision
            # info.opp_collision = self.opp_collision
            info.ego_elapsed_time = obs['lap_times'][0]
            # info.opp_elapsed_time = obs['lap_times'][1]
            info.ego_lap_count = obs['lap_counts'][0]
            # info.opp_lap_count = obs['lap_counts'][1]
...
class GymBridge(object):
    ...
    def __init__(self, scenario, map_path, ego_id, opp_id=None, map_image_ext=".png", plugins=[]):
        ...
        scan_fov, scan_beams = rospy.get_param(
            'scan_fov'), rospy.get_param('scan_beams')
        ids = [ego_id, opp_id]
        for ix, agent_id in enumerate(ids):
            if not agent_id:
                continue
            self.agents.append(
                Agent(agent_id, self._drive_callback(ix), scan_fov, scan_beams))
        ...
        if driver_count == 1:
            ...
            if 'SOCHI'.lower() in map_config_filename.lower():
                ...
                poses = np.array([[-13.9377183914, 4.44781494141, 2.79787]])
            elif 'oh16'.lower() in map_config_filename.lower():
                poses = np.array([[0.0, 0.0, 0.0]])
            else:
                raise ValueError(
                    "Initial position is unknown for map '{}'.".format(map_config_filename))
        elif driver_count == 2:
            if 'SOCHI'.lower() in map_config_filename.lower():
                poses = np.array([
                ...
                poses = np.array([
                    [-13.9377183914, 3.44781494141, 2.79787],
                    [-17.1113777161, 5.38383865356, 2.79787],
                ])
                ...
                raise ValueError(
                    "Initial positions are unknown for map '{}'.".format(map_config_filename))
        else:
            raise ValueError("Max 2 drivers are allowed")

        self.obs, self.rewards, self.done, _ = self.env.reset(
        ...
        self.drive_timer = rospy.Timer(rospy.Duration(
            self.DRIVE_TIMER_DURATION), self._drive_timer_callback)
    ...
    def _update(self):
        ...
        for i in range(len(self.agents)):
            obs = {single: self.obs[multi][i]
                   for multi, single in keys.items()}
...
if __name__ == '__main__':
    ...
    # import race map
    map_yaml = rospy.get_param("/gym_bridge/map_gym_yaml")
    rospack = rospkg.RosPack()
    race_map_path = rospack.get_path(
        "f1tenth_gym_ros") + "/maps" + "/" + map_yaml
    if not race_map_path:
        raise Exception("Map Path is not provided, cannot start simulation")

    # import drive id
    ego_id = rospy.get_param("/gym_bridge/ego_id")

    # import opponent id
    head_to_head_racing_enabled = rospy.get_param(
        "/gym_bridge/head_to_head", False)
    if(head_to_head_racing_enabled):
        opp_id = rospy.get_param("/gym_bridge/opp_id")
    else:
        opp_id = None
    ...
    gym_bridge = GymBridge(race_scenario, race_map_path,
                           ego_id, opp_id=opp_id, plugins=plugins)
    gym_bridge.spin()

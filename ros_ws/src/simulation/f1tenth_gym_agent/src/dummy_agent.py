...
class GapFollower:
    ...
    def __init__(self, name):
        ...
        self.pub_drive = rospy.Publisher('/%s/drive' % name, AckermannDriveStamped, queue_size=5)
        rospy.Subscriber('/%s/scan' % name, LaserScan, self.process_lidar)
...
if __name__ == "__main__":
    rospy.init_node('gym_agent_dummy', anonymous=True)
    name = rospy.get_param("/gym_agent_opponent/opp_id")
    agent = GapFollower(name=name)

    while not rospy.core.is_shutdown():

        rospy.rostime.wallsleep(0.04)

    print('Dummy agent is done.')